...
export default function LeadsClient({
  ...
  const refreshFromServer = useCallback(async () => {
    ...
    // wait briefly for server payload to reconcile
    setTimeout(() => {
  ...
  const addLead = useCallback(async () => {
    setErrorMsg(null);
    const { error } = await supabase.schema("leads").from("prospects").insert([
      {
        name: "Test Lead",
  ...
  // Keep local state in sync after server refresh/navigation
  useEffect(() => {
    ...
    () =>
      leads.map((lead) => (
        <tr key={lead.id}>
          ...
          <td>
            <button onClick={() => autoAssign(lead.id)}>Assign</button>
          </td>
        </tr>
      )),
    [leads, autoAssign]
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <p style={{ fontSize: 12, margin: "8px 0" }}>
        Realtime: <strong>{realtimeStatus}</strong>
      </p>
