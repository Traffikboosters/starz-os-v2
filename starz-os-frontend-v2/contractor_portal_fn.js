async function loadContractorContent(el) {
  ...
  try {
      ...
      return '<div class="p-4 rounded-xl border ' + phaseBg(stage) + ' cursor-pointer hover:opacity-90 transition-all" onclick="window._cpSelect(\'' + c.id + '\')">' +
        '<div class="flex items-start gap-3 mb-3">' +
          ...
          '<div class="flex-1 min-w-0">' +
            ...
            '<div class="text-[10px] text-slate-500">' + (comm?.email_address||'No corp email') + '</div>' +
          '</div>' +
        ...
        '<div class="grid grid-cols-4 gap-1 mb-3">' +
          ...
          '<div class="bg-black/20 rounded-lg p-1.5 text-center"><div class="text-xs font-bold font-mono text-green-400">' + (bge?.deals_closed||0) + '</div><div class="text-[8px] text-slate-600">Deals</div></div>' +
          '<div class="bg-black/20 rounded-lg p-1.5 text-center"><div class="text-xs font-bold font-mono text-amber-400">' + (bge?.close_rate ? (Number(bge.close_rate)*100).toFixed(0)+'%' : '—') + '</div><div class="text-[8px] text-slate-600">Close%</div></div>' +
        ...
        '<div class="flex gap-1 flex-wrap mb-2">' +
          (comm?.dialpad_phone ? '<span class="text-[9px] text-orange-300 bg-orange-500/10 rounded px-1.5 border border-orange-500/20">📞 ' + comm.dialpad_phone + '</span>' : '<span class="text-[9px] text-slate-600 bg-slate-700/20 rounded px-1.5">No Dialpad</span>') +
          (comm?.email_address ? '<span class="text-[9px] text-blue-300 bg-blue-500/10 rounded px-1.5 border border-blue-500/20">📧</span>' : '') +
          (bge?.leads_access ? '<span class="text-[9px] text-green-300 bg-green-500/10 rounded px-1.5 border border-green-500/20">🎯 ON</span>' : '<span class="text-[9px] text-red-400 bg-red-500/10 rounded px-1.5 border border-red-500/20">🎯 OFF</span>') +
        '</div>' +
    ...
    const renderPortal = () => {
      ...
      const leadRows = myLeads.length === 0 ? '<div class="text-center text-slate-600 py-4 text-xs">No leads assigned</div>' :
        myLeads.slice(0,25).map(l => {
          const statusCls = l.status==='ready' ? 'bg-green-500/15 border-green-500/25 text-green-400' : l.status==='calling' ? 'bg-orange-500/15 border-orange-500/25 text-orange-400' : 'bg-slate-700/30 border-slate-700/40 text-slate-500';
          return '<div class="flex items-center gap-2 p-2 rounded-lg bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.06] transition-all">' +
            ...
            '<span class="text-[8px] px-1.5 py-0.5 rounded border ' + statusCls + '">' + (l.status||'ready').toUpperCase() + '</span>' +
            '<button onclick="window._cpDialLead(\'' + l.phone + '\',\'' + (comm?.dialpad_user_id||'4566842998267904') + '\',\'' + l.id + '\')" class="p-1 rounded bg-green-500/20 text-green-300 text-[9px] hover:bg-green-500/30">📞</button>' +
      ...
      const commRows = myComms.slice(0,4).map(cm =>
        '<div class="flex items-center justify-between text-[10px] py-1 border-b border-white/[0.04]">' +
      ...
      return '<div class="space-y-4">' +
          ...
          '<div class="flex items-center gap-4">' +
            ...
            '<div class="flex-1">' +
              ...
              '<div class="flex gap-2 mt-1 flex-wrap">' +
                (comm?.email_address ? '<span class="text-[10px] text-blue-300 bg-blue-500/10 border border-blue-500/20 rounded px-2 py-0.5">📧 ' + comm.email_address + '</span>' : '') +
                (comm?.dialpad_phone ? '<span class="text-[10px] text-orange-300 bg-orange-500/10 border border-orange-500/20 rounded px-2 py-0.5">📞 ' + comm.dialpad_phone + '</span>' : '') +
            ...
            '<div class="grid grid-cols-3 gap-2">' +
              ...
              '<div class="text-center bg-black/20 rounded-lg p-2"><div class="text-base font-black font-mono text-green-400">' + (bge?.deals_closed||0) + '</div><div class="text-[8px] text-slate-600">Deals</div></div>' +
        ...
        '<div class="grid grid-cols-2 gap-4">' +
          '<div class="space-y-3">' +
            ...
            '<button onclick="window._cpAutoDial(\'' + c.email + '\',\'' + (comm?.dialpad_user_id||'4566842998267904') + '\')" class="py-1 px-3 rounded-lg bg-green-500/20 border border-green-500/30 text-green-300 text-[10px] font-semibold hover:bg-green-500/30">▶ Auto-Dial</button></div>' +
            '<div class="flex gap-2"><input id="cp-manual-' + c.id + '" type="text" placeholder="Manual dial…" class="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white placeholder-slate-600 outline-none" onkeydown="if(event.key===\'Enter\')window._cpManualDial(\'' + c.id + '\',\'' + (comm?.dialpad_user_id||'4566842998267904') + '\')" />' +
            '<button onclick="window._cpManualDial(\'' + c.id + '\',\'' + (comm?.dialpad_user_id||'4566842998267904') + '\')" class="px-3 py-1.5 rounded-lg bg-green-500/20 border border-green-500/30 text-green-300 text-xs font-semibold hover:bg-green-500/30">📞</button></div>' +
            '<div class="space-y-1 max-h-64 overflow-y-auto">' + leadRows + '</div>' +
          '</div>' +
          '<div class="space-y-3">' +
            '<div class="text-xs font-semibold text-slate-300">🤖 AI Agents</div>' +
            '<div class="grid grid-cols-3 gap-2">' +
              '<button onclick="window._cpOpenChat(\'steve\',\'' + c.id + '\')" class="p-3 rounded-xl border text-left transition-all hover:opacity-90" style="background:rgba(139,92,246,0.08);border-color:rgba(139,92,246,0.25)">' +
                '<img src="https://auth.starzcrm.traffikboosters.com/storage/v1/object/public/starz-ai-agents/AI%20AGENTS/Steve.png" class="w-10 h-10 rounded-full object-cover mb-2" onerror="this.style.display=\'none\'" />' +
                '<div class="text-xs font-bold text-white">Steve BGE</div><div class="text-[8px] text-slate-500">Sales Coach</div></button>' +
              '<button onclick="window._cpOpenChat(\'zara\',\'' + c.id + '\')" class="p-3 rounded-xl border text-left transition-all hover:opacity-90" style="background:rgba(244,63,94,0.08);border-color:rgba(244,63,94,0.25)">' +
                '<img src="https://auth.starzcrm.traffikboosters.com/storage/v1/object/public/starz-ai-agents/AI%20AGENTS/Zara.png" class="w-10 h-10 rounded-full object-cover mb-2" onerror="this.style.display=\'none\'" />' +
                '<div class="text-xs font-bold text-white">Zara HR</div><div class="text-[8px] text-slate-500">HR Director</div></button>' +
              '<button onclick="window._cpOpenChat(\'rico\',\'' + c.id + '\')" class="p-3 rounded-xl border text-left transition-all hover:opacity-90" style="background:rgba(14,165,233,0.08);border-color:rgba(14,165,233,0.25)">' +
                '<img src="https://auth.starzcrm.traffikboosters.com/storage/v1/object/public/starz-ai-agents/AI%20AGENTS/Rico.png" class="w-10 h-10 rounded-full object-cover mb-2" onerror="this.style.display=\'none\'" />' +
                '<div class="text-xs font-bold text-white">Rico BGE</div><div class="text-[8px] text-slate-500">Ops Director</div></button>' +
            '</div>' +
            '<div id="cp-chat-' + c.id + '" class="hidden space-y-2">' +
              '<div class="flex items-center gap-2"><span id="cp-agent-label-' + c.id + '" class="text-xs font-bold text-white"></span><button onclick="document.getElementById(\'cp-chat-' + c.id + '\').classList.add(\'hidden\')" class="ml-auto text-slate-500 text-xs">✕</button></div>' +
              '<div id="cp-msgs-' + c.id + '" class="bg-black/20 rounded-xl p-3 h-28 overflow-y-auto space-y-1.5 text-xs text-slate-300"><div class="text-slate-600 italic text-[10px]">Ask your AI agent…</div></div>' +
              '<div class="flex gap-2"><input id="cp-input-' + c.id + '" type="text" placeholder="Message…" class="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white placeholder-slate-600 outline-none" onkeydown="if(event.key===\'Enter\')window._cpSendChat(\'' + c.id + '\')" />' +
              '<button onclick="window._cpSendChat(\'' + c.id + '\')" class="px-3 py-1.5 rounded-lg text-xs font-bold text-white" style="background:linear-gradient(135deg,#6366f1,#8b5cf6)">Send</button></div>' +
            '</div>' +
            '<div class="grid grid-cols-2 gap-2">' +
              '<button onclick="' + (comm?.video_room_url ? 'window.open(\'' + comm.video_room_url + '\',\'_blank\')' : 'showRealtimeToast(\'No video room\',\'amber\')') + '" class="p-3 rounded-xl bg-blue-500/10 border border-blue-500/20 text-left hover:bg-blue-500/15 transition-all"><div class="text-lg mb-1">🎥</div><div class="text-xs font-semibold text-blue-300">Video</div><div class="text-[9px] text-slate-500">' + (comm?.video_room_url?'Ready':'Not set') + '</div></button>' +
              '<button onclick="window.open(\'https://mail.google.com\',\'_blank\')" class="p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-left hover:bg-rose-500/15 transition-all"><div class="text-lg mb-1">📧</div><div class="text-xs font-semibold text-rose-300">Email</div><div class="text-[9px] text-slate-500 truncate">' + (comm?.email_address||'Not assigned') + '</div></button>' +
            '</div>' +
    ...
    const renderLeaderboard = () => {
      ...
      const rows = ranked.map((c,i) => {
        ...
        const comm = getComms(c.auth_user_id);
        const e = earnedAmt(c.auth_user_id);
        const cls = i===0?'text-amber-400':i===1?'text-slate-300':i===2?'text-amber-700':'text-slate-500';
        return '<tr class="' + (i===0?'bg-amber-500/5':'') + '">' +
          '<td class="font-mono font-bold ' + cls + '">' + (medals[i]||i+1) + '</td>' +
          '<td class="font-semibold">' + (c.full_name||'—') + '</td>' +
          '<td class="text-xs text-blue-300">' + (comm?.email_address||'—') + '</td>' +
          '<td class="font-mono text-xs text-orange-300">' + (comm?.dialpad_phone||'—') + '</td>' +
          '<td class="font-mono text-cyan-400">' + getLeads(c.email).length + '</td>' +
          '<td class="font-mono text-green-400 font-bold">' + (bge?.deals_closed||0) + '</td>' +
          '<td class="font-mono text-amber-400">' + (bge?.close_rate ? (Number(bge.close_rate)*100).toFixed(1)+'%' : '—') + '</td>' +
          '<td class="font-mono text-emerald-400 font-bold">\$' + Number(bge?.revenue_generated||0).toLocaleString() + '</td>' +
          '<td class="font-mono text-purple-400">\$' + e.toLocaleString() + '</td>' +
    ...
    const renderKPI = () => {
      const cards = contractors.map(c => {
        ...
        ].map(m => '<div class="flex items-center gap-2"><span class="text-[9px] text-slate-500 w-14 shrink-0">' + m.l + '</span><div class="flex-1 h-1 bg-slate-700 rounded-full overflow-hidden"><div class="h-full bg-indigo-400 rounded-full" style="width:' + m.v + '%"></div></div><span class="text-[9px] text-slate-400 font-mono w-6 text-right">' + m.v + '</span></div>').join('');
        return '<div class="p-4 rounded-xl border border-white/10 bg-white/[0.02]">' +
          '<div class="flex items-center gap-2 mb-3"><div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white" style="background:linear-gradient(135deg,#6366f1,#8b5cf6)">' + initial + '</div>' +
          ...
          '<div class="mb-3"><div class="flex justify-between text-[10px] mb-1"><span class="text-slate-500">Performance</span><span class="font-bold" style="color:' + sc + '">' + score.toFixed(0) + '/100</span></div>' +
          ...
          '</div>' +
          (bge?.assignment_paused ? '<div class="mt-2 text-[9px] text-red-400 bg-red-500/10 rounded px-2 py-1 text-center">⚠ Paused</div>' : '') +
        '</div>';
      }).join('');
    ...
    const renderCP = () => {
      const tabHTML = tabs.map(t => '<button onclick="window._cpTab(\'' + t.id + '\')" class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ' + (activeTab===t.id ? 'bg-indigo-500/30 text-white border border-indigo-500/40' : 'text-slate-400 hover:text-white hover:bg-white/5') + '">' + t.label + '</button>').join('');
      const selectorHTML = (activeTab==='portal' && contractors.length>1) ? '<div class="ml-4 flex items-center gap-2"><span class="text-[10px] text-slate-500">Viewing:</span><select onchange="window._cpSelectByIdx(this.value)" class="bg-black/30 border border-white/10 rounded-lg px-2 py-1 text-[10px] text-white outline-none">' + contractors.map((c,i) => '<option value="' + i + '"' + (selectedContractor?.id===c.id?' selected':'') + '>' + (c.full_name||c.email) + '</option>').join('') + '</select></div>' : '';
      el.innerHTML = '<div class="flex flex-col h-full">' +
        '<div class="flex items-center gap-3 p-4 rounded-2xl border border-indigo-500/20 mb-4 shrink-0" style="background:linear-gradient(135deg,rgba(99,102,241,0.08),rgba(139,92,246,0.04))">' +
          ...
          '</div>' +
        '</div>' +
        '<div class="flex items-center gap-1 mb-3 shrink-0 flex-wrap">' + tabHTML + selectorHTML + '</div>' +
    ...
    window._cpSelectByIdx = (i) => { selectedContractor=contractors[Number(i)]||null; renderCP(); };
    ...
    window._cpDialLead = async (phone, dialpadUserId, queueId) => {
      ...
      try {
        ...
        if (data?.error) { showRealtimeToast(data.error,'red'); return; }
      ...
      } catch(e) { showRealtimeToast(e.message,'red'); }
    };
    ...
    window._cpSendChat = async (contractorId) => {
      ...
      if (input) input.value = '';
      const panel = document.getElementById('cp-chat-'+contractorId);
      const agentId = panel?.dataset?.agent || 'steve';
      const msgsEl = document.getElementById('cp-msgs-'+contractorId);
      if (msgsEl) { msgsEl.innerHTML += '<div class="text-right text-white bg-indigo-500/20 rounded px-2 py-1 text-[10px]">'+msg+'</div><div id="cp-typing-'+contractorId+'" class="text-slate-400 text-[10px] italic">thinking…</div>'; msgsEl.scrollTop=msgsEl.scrollHeight; }
      const systems = {
        steve:'You are Steve BGE, elite sales coach at Traffik Boosters. Help with objection handling, closing, scripts for digital marketing (SEO, Google Ads, websites, AI automation). Commission: 30% + residuals. Be sharp and tactical.',
        zara:'You are Zara, HR Director at Traffik Boosters. Help with HR questions, policies, lead rules (24h contact, 3-day inactivity=reassign, 10-day close window, 70 lead cap), and Florida employment law.',
        rico:'You are Rico, Ops Director at Traffik Boosters. Help with post-sale process, fulfillment timelines, work orders, and service delivery (SEO, Ads, Website, AI, Lead Gen, Analytics, Content, Voice AI).'
      };
      try {
      ...
      } catch(e) { const t=document.getElementById('cp-typing-'+contractorId); if(t) t.textContent='Error: '+e.message; }
    };
