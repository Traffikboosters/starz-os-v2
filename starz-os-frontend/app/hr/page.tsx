...
function StatusBadge({ status }: { status: string | null }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${c[status?.toLowerCase() || ''] || 'text-gray-400 bg-gray-400/10'}`}>
      {status || '-'}
    </span>
  );
}
...
export default function HRPortal() {
  ...
  async function generateOnboardingEmail(user: HRUser) {
    ...
    try {
      ...
      setSendError('Failed to generate email. Please try again.');
    }
    setGenerating(false);
  }
        ...
        <>
          ...
          {tab === 'staff' && (
            <div className="overflow-x-auto rounded-xl border border-gray-800">
              {users.length === 0 ? (
                <div className="py-20 text-center text-gray-500 text-sm">No staff found</div>
              ) : (
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {users.filter(u => !q || u.full_name?.toLowerCase().includes(q) || u.email?.toLowerCase().includes(q)).map(u => (
                      <tr key={u.id} className="hover:bg-gray-900/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <button onClick={() => { setEmailTarget(u); setShowEmailModal(true); }}
                            className="flex items-center gap-1 px-3 py-1.5 bg-purple-500/20 hover:bg-purple-500/30 text-purple-400 rounded-lg text-xs font-medium transition-colors">
                            <Mail className="w-3 h-3" />Onboard
                          </button>
                        </td>
                      </tr>
                    ))}
          ...
          {tab === 'compliance' && (
            <div className="overflow-x-auto rounded-xl border border-gray-800">
              {compliance.length === 0 ? (
                <div className="py-20 text-center text-gray-500 text-sm">No compliance rules found</div>
              ) : (
                <table className="w-full text-sm">
                  <thead className="bg-gray-900 text-gray-400 text-xs uppercase">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left">Min Wage</th>
                      <th className="px-4 py-3 text-left">Overtime Policy</th>
                      <th className="px-4 py-3 text-left">Worker Types</th>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {compliance.filter(c => !q || c.state_code.toLowerCase().includes(q) || c.role_key.toLowerCase().includes(q)).map(c => (
                      <tr key={c.id} className="hover:bg-gray-900/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          {c.requires_overtime_policy
                            ? <span className="text-green-400 text-xs flex items-center gap-1"><CheckCircle className="w-3 h-3" />Required</span>
                            : <span className="text-gray-500 text-xs">Not required</span>}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {(c.allowed_worker_types || []).map(t => (
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {invites.filter(i => !q || i.email?.toLowerCase().includes(q) || i.role_key?.toLowerCase().includes(q)).map(i => (
                      <tr key={i.id} className="hover:bg-gray-900/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-0.5 rounded-full text-xs bg-purple-400/10 text-purple-400 capitalize">{i.role_key || '-'}</span>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-gray-900 border border-gray-700 rounded-2xl p-6 z-10 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center gap-3 mb-6">
              <img src={ZARA_AVATAR} alt="Zara" className="w-10 h-10 rounded-xl object-cover" />
              <div>
                <h2 className="text-lg font-bold text-white">Send Onboarding Email</h2>
                <p className="text-gray-400 text-sm">To: {emailTarget.full_name} &middot; {emailTarget.email}</p>
              </div>
              <button onClick={closeModal} className="ml-auto text-gray-400 hover:text-white text-xl leading-none">&times;</button>
            </div>
            ...
            {emailSent ? (
              <div className="text-center py-8">
                ...
                <button onClick={closeModal} className="mt-4 px-4 py-2 bg-purple-500/20 text-purple-400 rounded-lg text-sm hover:bg-purple-500/30 transition-colors">Close</button>
              </div>
            ) : (
              <>
                {sendError && (
                ...
                {!generatedEmail ? (
                  <div className="text-center py-8">
                    <p className="text-gray-400 text-sm mb-6">
                      Zara will generate a personalized onboarding email using AI based on {emailTarget.full_name}&apos;s role and profile.
                    </p>
                    <button onClick={() => generateOnboardingEmail(emailTarget)} disabled={generating}
                      className="px-6 py-3 bg-purple-500 hover:bg-purple-600 disabled:opacity-50 text-white rounded-xl text-sm font-semibold flex items-center gap-2 mx-auto transition-colors">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-2">
                      <button onClick={() => { setGeneratedEmail(null); setSendError(null); }}
                        className="flex-1 px-4 py-2.5 border border-gray-700 text-gray-400 hover:text-white rounded-xl text-sm transition-colors">
                        Regenerate
                      </button>
                      <button onClick={sendOnboardingEmail} disabled={emailLoading}
                        className="flex-1 px-4 py-2.5 bg-purple-500 hover:bg-purple-600 disabled:opacity-50 text-white rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors">
