...
import { X, Send, ClipboardList, Users, Cpu, Package } from 'lucide-react';
...
const SUGGESTIONS = [
  ...
  { icon: Users, text: 'Who is available?' },
  { icon: Package, text: 'Track deliverable' },
];

const DEFAULT_RESPONSE = `Greetings! I'm Rico, your Technical Supervisor BGE.
...
export function RicoChat() {
  ...
  const handleSend = async (text: string = input) => {
    ...
    try {
      const res = await fetch('/api/rico-chat', {
        method: 'POST',
  ...
  return (
    <>
      <button
        ...
        className={`fixed bottom-6 left-6 z-50 w-16 h-16 rounded-full overflow-hidden hover:scale-110 transition-all ring-2 ring-cyan-400 ring-offset-2 ring-offset-[#0a0a0f] shadow-[0_0_20px_rgba(34,211,238,0.5)] ${isOpen ? 'scale-0' : 'scale-100'}`}
      >
      ...
      {isOpen && (
        <div className="fixed bottom-6 left-6 z-50 w-96 rounded-2xl shadow-2xl border border-white/10 bg-gray-900">
          <div className="flex items-center gap-3 p-4 border-b border-white/10">
            <div className="w-10 h-10 rounded-full overflow-hidden ring-2 ring-cyan-500/40">
              <img src={RICO_AVATAR} alt="Rico" className="w-full h-full object-cover" />
            </div>
          ...
          <div className="h-72 overflow-y-auto p-4 space-y-4">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${msg.role === 'assistant' ? 'bg-white/5 text-white/90' : 'bg-cyan-600 text-white'}`}>
          ...
          <div className="p-3 border-t border-white/10">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/40 outline-none focus:border-cyan-500/50"
              />
              <button
                onClick={() => handleSend()}
                disabled={isTyping}
                className="p-2.5 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 rounded-xl text-white transition-colors"
              >
