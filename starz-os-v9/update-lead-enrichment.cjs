...
// ── RPC: trigger-safe enrichment write ───────────────────────────────────────
async function applyEnrichment(leadId: string, enriched: any, score: number, industry: string) {
  const priority   = score >= 80 ? 'critical' : score >= 60 ? 'high' : 'low'
  const disposition = score >= 80 ? 'hot_handoff' : score >= 60 ? 'handoff_ready' : 'nurture'
  const status     = score >= 60 ? 'qualified' : 'unqualified'
  const action     = score >= 80 ? 'call_now' : score >= 60 ? 'email_followup' : 'skip'
  const notes      = generateAiNotes(score, enriched, industry)

  const res = await fetch(\`\${SB_URL}/rest/v1/rpc/apply_lead_enrichment\`, {
    method: 'POST',
    headers: { apikey: SB_KEY, Authorization: 'Bearer ' + SB_KEY, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      p_lead_id:         leadId,
      p_website:         enriched.website         || null,
      p_rating:          enriched.rating           || null,
      p_review_count:    enriched.review_count     || null,
      p_score:           score,
      p_priority_level:  priority,
      p_disposition:     disposition,
      p_status:          status,
      p_next_best_action: action,
      p_ai_notes:        notes,
    })
  })
  ...
  if (!res.ok) {
    ...
    console.error('RPC apply_lead_enrichment failed:', err)
    return false
  }
  return true
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Select leads needing enrichment
    const filter = forceAll
      ? \`status=eq.qualified&limit=\${batchSize}&order=score.desc\`
      : \`status=eq.qualified&website=is.null&rating=is.null&enriched_at=is.null&limit=\${batchSize}&order=score.desc\`

    const leads = await dbGet('public', 'leads',
      \`select=id,business_name,phone,industry,score&\${filter}\`
    )
    ...
    for (const lead of leads) {
      ...
      const success = await applyEnrichment(lead.id, enriched, score, lead.industry)
...
console.log('✅ Updated lead-enrichment/index.ts with trigger-safe RPC writes');
console.log('\nDeploy:');
